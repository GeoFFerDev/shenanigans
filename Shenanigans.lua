--[[
  IMP HUB X  v6  —  Jujutsu Shenanigans
  Template: Fluent Local UI Framework (exact match)
...
-- ══════════════════════════════════════════════
-- GAME LOGIC  (defined FIRST — no forward refs)
-- ══════════════════════════════════════════════
...
local function FaceTarget(t)
    ...
    if r and tr then r.CFrame=CFrame.new(r.Position,Vector3.new(tr.Position.X,r.Position.Y,tr.Position.Z)) end
end
local orbitAngle=0
local function OrbitTarget(t)
    local r,tr=Root(LP),Root(t); if not r or not tr then return end
...
local function MoveToTarget(t)
    ...
    else TweenService:Create(r,TweenInfo.new(
...
local function SetWhiteScreen(on)
    ...
    if on then
        ...
        f.BackgroundColor3=Color3.new(1,1,1); f.BackgroundTransparency=0.35; f.BorderSizePixel=0
    end
end
local ctpConn
local function SetClickTP(on)
    ...
    if on then ctpConn=UserInputService.InputBegan:Connect(function(inp,gp)
        ...
        and UserInputService:IsKeyDown(Enum.KeyCode.LeftControl) then
            local r=Root(LP); if r then r.CFrame=CFrame.new(Mouse.Hit.Position+Vector3.new(0,3,0)) end
        end
    end) end
end
local spinConn
...
local specConn
local function SpectatePlayer(t)
...
-- ══════════════════════════════════════════════
-- THEME  (template colours, accent = purple)
-- ══════════════════════════════════════════════
local Theme={
...
ToggleIcon.Text="⚔"; ToggleIcon.TextSize=22
ToggleIcon.TextColor3=Theme.Text; ToggleIcon.Font=Enum.Font.GothamBold
...
local _mstk=Instance.new("UIStroke",MainFrame)
_mstk.Color=Theme.Accent; _mstk.Transparency=0.5
...
Title.Font=Enum.Font.GothamMedium; Title.TextColor3=Theme.Text
Title.TextSize=11; Title.TextXAlignment=Enum.TextXAlignment.Left
Title.BackgroundTransparency=1

-- Window controls (exact template sizes: 30×20)
local function AddControl(text,pos,col,cb)
    ...
    btn.Text=text; btn.TextColor3=col
...
-- ══════════════════════════════════════════════
-- SIDEBAR  (exact template: 110px wide)
-- ══════════════════════════════════════════════
...
-- NOTE: no ClipsDescendants here — keep exactly like template

-- ══════════════════════════════════════════════
-- DROPDOWN OVERLAY (parented to ScreenGui, ZIndex above all)
-- Only becomes interactive when a dropdown is open
-- ══════════════════════════════════════════════
...
DropOverlay.Active=false   -- NOT active by default — won't eat clicks
...
local function CreateTab(name,icon)
    ...
    -- Fallback for Delta: update CanvasSize manually too
...
-- Section label (thin, accent-coloured)
local function MkSection(parent,text,order)
    local f=Instance.new("Frame",parent)
    f.Size=UDim2.new(0.98,0,0,18); f.BackgroundTransparency=1; f.LayoutOrder=order
    local l=Instance.new("TextLabel",f)
    l.Size=UDim2.new(1,0,1,0); l.BackgroundTransparency=1
    l.Text=string.upper(text); l.Font=Enum.Font.GothamBold
    l.TextSize=9; l.TextColor3=Theme.AccentLt; l.TextXAlignment=Enum.TextXAlignment.Left
    -- underline
...
local function MkToggle(parent,title,desc,default,order,cb)
    ...
    PTxt.Text=state and "ON" or "OFF"; PTxt.Font=Enum.Font.GothamBold; PTxt.TextSize=10
...
local function MkButton(parent,title,order,cb)
    ...
    btn.Text="  "..title; btn.TextColor3=Theme.Text
...
UserInputService.InputEnded:Connect(function(i)
    if i.UserInputType==Enum.UserInputType.MouseButton1
    or i.UserInputType==Enum.UserInputType.Touch then sliderActive=nil end
end)
...
local function MkSlider(parent,title,mn,mx,def,sfx,order,cb)
    ...
    card.BackgroundColor3=Theme.Button; card.BorderSizePixel=0; card.LayoutOrder=order
...
-- Dropdown  (compact: 38px; list floats in DropOverlay)
local function MkDrop(parent,title,opts,def,order,cb)
    ...
    -- List in DropOverlay so it's never clipped
        ...
        ob.MouseButton1Click:Connect(function()
            ...
            for _,c in ipairs(List:GetChildren()) do
                if c:IsA("TextButton") then
                    c.TextColor3=c.Text:gsub("^%s+","")==opt and Theme.AccentLt or Theme.Text
                end
            end
            pcall(cb,sel)
        end)
    ...
    DBtn.MouseButton1Click:Connect(function()
        ...
        DropOverlay.Active=true   -- now intercepts outside clicks
    end)

    return function() return sel end
end

-- Status card (shows farm active/inactive)
...
local farmTargetM = MkDrop(TFarm,"Target Method",
...
MkToggle(TFarm,"Fast Attack","Spam attack remote",true,4,
    function(v) Cfg.Farm.FastAttack=v end)

MkToggle(TFarm,"Use Skills","Fire skill keybinds",true,5,
...
local getDetR=MkSlider(TFarm,"Detection Range",5,80,20," studs",13,
    function(v) Cfg.Block.DetectRange=v end)

local getBlkD=MkSlider(TFarm,"Block Delay",0,5,0,"s",14,
...
local getAimMode=MkDrop(TFarm,"Aimlock Mode",{"Camera","Body","Silent"},"Camera",17,
    function(v) Cfg.Aim.Mode=v end)

local getAimPart=MkDrop(TFarm,"Target Part",{"Head","HumanoidRootPart","Torso"},"Head",18,
...
local getCTpM=MkDrop(TCombat,"Teleport Method",{"Tween","Instant","Lerp"},"Tween",2,
    function(v) Cfg.Combat.TpMethod=v end)

local getCMove=MkDrop(TCombat,"Movement Mode",{"Orbit (Dodge)","Follow","Static"},"Orbit (Dodge)",3,
...
MkToggle(TCombat,"Smart Kiting (Retreat on CD)","Dodge back on skill CD",true,6,
    function(v) Cfg.Combat.SmartKiting=v end)

MkSection(TCombat,"Main Configurations",7)

MkToggle(TCombat,"Auto Flee (Low HP)","Retreat when HP is low",false,8,
    function(v) Cfg.Combat.AutoFlee=v end)

MkSlider(TCombat,"Flee Health %",5,80,20,"%",9,
    function(v) Cfg.Combat.FleeHP=v end)

MkToggle(TCombat,"Priority Closest","Always target nearest enemy",true,10,
    function(v) Cfg.Combat.PriorClosest=v end)

MkToggle(TCombat,"Hunter Mode","Chase fleeing targets",false,11,
    function(v) Cfg.Combat.HunterMode=v end)

MkSection(TCombat,"Skill System",12)

MkDrop(TCombat,"Select Skills",
    {"Divergent Fist","Black Flash","Hollow Purple","Domain Expansion","All"},
    "Divergent Fist",13, function(v) Cfg.Combat.SkillName=v end)

MkSlider(TCombat,"Skill Delay",0,30,0,"s",14,
    function(v) Cfg.Combat.SkillDelay=v end)

MkToggle(TCombat,"Avoid Skills (No Target)","Skip skills needing a target",true,15,
    function(v) Cfg.Combat.AvoidNoTarget=v end)

MkToggle(TCombat,"Semi Kill Aura (25 Studs)","Auto-attack nearby enemies",false,16,
    function(v) Cfg.Combat.SemiKillAura=v; SetKillAura(v) end)

MkToggle(TCombat,"SpinBot","Continuously rotate character",false,17,
...
MkToggle(TESP,"Box","Draw bounding box",true,4,
    function(v) Cfg.ESP.Box=v end)

MkToggle(TESP,"Tracers","Draw tracer lines",true,5,
    function(v) Cfg.ESP.Tracers=v end)

MkToggle(TESP,"Health Bar","Show health bar",true,6,
    function(v) Cfg.ESP.HealthBar=v end)

MkToggle(TESP,"Distance","Show stud distance",true,7,
    function(v) Cfg.ESP.Distance=v end)

MkToggle(TESP,"Name","Show player name",true,8,
    function(v) Cfg.ESP.Name=v end)

MkToggle(TESP,"Moveset (Class)","Show character moveset",true,9,
    function(v) Cfg.ESP.Moveset=v end)
...
MkToggle(TMisc,"WalkSpeed Bypass (Velocity)","Speed via BodyVelocity",false,4,
...
MkToggle(TMisc,"White Screen","White overlay on screen",false,8,
...
InfoLine(TCreds,"v6  •  Delta Compatible  •  400×260",Theme.SubText,3)
InfoLine(TCreds,"⚔ Farm | ⚙ Combat | ● ESP | ★ Misc",Theme.SubText,4)
InfoLine(TCreds,"Toggle GUI: RightShift",Theme.AccentLt,5)
InfoLine(TCreds,"For educational purposes only.",Theme.SubText,6)
...
RunService.Heartbeat:Connect(function()
    ...
    if Cfg.Misc.AntiAFK and now-lastAfk>60 then
        lastAfk=now; pcall(function() local h=Hum(LP); if h then h.Jump=false end end)
    end
...
local function GetMoveset(p)
    ...
    if ch then for _,n in ipairs({"Moveset","Class","Style","Weapon"}) do
        local v=ch:FindFirstChild(n); if v and v.Value then return tostring(v.Value) end
    end end
...
local function MakeESP(p)
    ...
    for i=1,4 do
        ...
        l.Color=Color3.fromRGB(130,45,235); l.Visible=false; l.ZIndex=2; o.Box[i]=l
    end
    o.Tracer=Drawing.new("Line"); o.Tracer.Thickness=1
    o.Tracer.Color=Color3.fromRGB(130,45,235); o.Tracer.Visible=false
    ...
    o.Name=Drawing.new("Text"); o.Name.Size=13; o.Name.Color=Color3.new(1,1,1)
    o.Name.Center=true; o.Name.Outline=true; o.Name.Visible=false
    o.Dist=Drawing.new("Text"); o.Dist.Size=11; o.Dist.Color=Color3.fromRGB(200,200,200)
    o.Dist.Center=true; o.Dist.Outline=true; o.Dist.Visible=false
    o.Move=Drawing.new("Text"); o.Move.Size=11; o.Move.Color=Color3.fromRGB(170,125,255)
    o.Move.Center=true; o.Move.Outline=true; o.Move.Visible=false
    ESPObjs[p]=o
end
        ...
        if not p.Parent or not Alive(p) then RemESP(p)
            ...
            if not r or not h then HideESP(o)
                ...
                if not vis or dep<=0 then HideESP(o)
                    ...
                    local corners={{Vector2.new(L,T),Vector2.new(R,T)},
                    ...
                    o.HpBg.Size=Vector2.new(4,ht); o.HpBg.Position=Vector2.new(L-7,T)
                    o.HpBg.Visible=Cfg.ESP.HealthBar
                    o.Hp.Size=Vector2.new(4,ht*pct); o.Hp.Position=Vector2.new(L-7,T+ht*(1-pct))
                    o.Hp.Color=Color3.fromRGB(math.floor(255*(1-pct)),math.floor(255*pct),0)
                    o.Hp.Visible=Cfg.ESP.HealthBar
                    o.Name.Text=p.Name; o.Name.Position=Vector2.new(sp.X,T-16)
                    o.Name.Visible=Cfg.ESP.Name
                    o.Dist.Text=math.floor(Dist(p)).."st"; o.Dist.Position=Vector2.new(sp.X,B+3)
                    o.Dist.Visible=Cfg.ESP.Distance
                    o.Move.Text="["..GetMoveset(p).."]"; o.Move.Position=Vector2.new(sp.X,B+15)
...
print("[ImpHubX v6] Loaded via "..tostring(TargetParent).."  |  RightShift = toggle")
