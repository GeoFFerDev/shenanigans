...
local Knit, ToolController, BlockService
local function InitKnitRefs()
    if ToolController or BlockService then return true end
    if not Knit then
        local ok, mod = pcall(function()
            return require(game.ReplicatedStorage:WaitForChild("Knit"):WaitForChild("Knit"))
        end)
        if not ok or not mod then return false end
        Knit = mod
    end

    pcall(function() ToolController = Knit.GetController("ToolController") end)
    pcall(function() BlockService = Knit.GetService("BlockService") end)
    return ToolController ~= nil or BlockService ~= nil
end

local function CharacterBusy(char)
    if not char then return true end
    local ragdoll = char:GetAttribute("Ragdoll")
    if ragdoll == true then return true end
    if type(ragdoll) == "number" and ragdoll > 0 then return true end
    if char:GetAttribute("Stun") then return true end
    if char:GetAttribute("Emote") then return true end
    return false
end
...
local function DoAttack()
    ...
    if CharacterBusy(char) then return end

    if InitKnitRefs() and ToolController and ToolController.UseSetService then
        local hum = char and char:FindFirstChildOfClass("Humanoid")
        local arg = nil
        if hum then
            if hum.FloorMaterial == Enum.Material.Air then arg = "Down"
            elseif hum.Jump then arg = "Up" end
        end
        pcall(function()
            if arg then ToolController:UseSetService("Activated", arg)
            else ToolController:UseSetService("Activated") end
        end)
        return
    end
...
local function SetBlock(s)
    ...
    if InitKnitRefs() and BlockService then
        pcall(function()
            if s then
                local tgt = nil
                if ToolController then
                    if ToolController.GetTargetOrProjectile then
                        tgt = ToolController:GetTargetOrProjectile()
                    elseif ToolController.GetTarget then
                        tgt = ToolController:GetTarget()
                    end
                end
                BlockService.Activated:Fire(tgt)
            else
                BlockService.Deactivated:Fire()
            end
        end)
        return
    end
...
local function UseSkill(t)
    ...
    local slot = math.random(1, 4)
    if InitKnitRefs() and ToolController and ToolController.UseKey then
        lastSkill = tick()
        pcall(function() ToolController:UseKey(slot, "Activated") end)
        task.delay(0.08, function()
            pcall(function() ToolController:UseKey(slot, "Deactivated") end)
        end)
        return
    end
...
MkToggle(TFarm,"Face Attacker","Look at attacker while blocking",true,12,function(v) Cfg.Block.FaceAttacker=v end)
    ...
    pcall(function()
        if Cfg.Farm.Enabled then
            ...
            if t then
                ...
                if CharacterBusy(myChar) then return end
                local h = Hum(LP)
